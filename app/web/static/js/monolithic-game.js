...
function startMonoGame(review = null) {
  ...
  monoSession = {questions, index: 0, correct: 0, wrong: [], locked: false, review: Boolean(review)};
...
function renderMonoQuestion() {
  ...
  if (!question) {
    ...
    document.querySelector("#monoReview")?.addEventListener("click", () => startMonoGame(shuffleItems(session.wrong)));
    document.querySelector("#monoAgain").onclick = () => startMonoGame();
