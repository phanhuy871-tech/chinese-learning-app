...
async function switchMode(mode) {
  ...
  if (mode === "monolithic" && !monolithicCards.length) {
...
function filteredMonolithic() {
  const week = Number(document.querySelector("#monoWeek")?.value || 0);
  const coreOnly = document.querySelector("#monoCore")?.checked ?? true;
...
function renderMonolithicList() {
  ...
  list.innerHTML = items.map((item, index) => `
    <button class="mono-item" type="button" data-mono-index="${monolithicCards.indexOf(item)}">
...
function evolutionCard(label, url, fallback) {
  return `<figure><div class="evolution-image"><img src="${escapeHtml(url)}" alt="${escapeHtml(label)}" loading="lazy" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span hidden>${escapeHtml(fallback)}</span></div><figcaption>${escapeHtml(label)}</figcaption></figure>`;
}
...
function selectMonolithic(index) {
  ...
  document.querySelector("#monolithicDetail").innerHTML = `
    <article class="mono-detail">
      ...
      <details><summary>Nguồn gốc và tiến hóa chữ</summary><p class="origin-source-note"><strong>${escapeHtml(item.etymology_type || "Tư liệu chữ cổ")}</strong> · Các diễn giải hình thể dưới đây dựa trên dữ liệu tiến hóa chữ của Academia Sinica và chú giải Hán tự; một số điểm là giả thuyết học thuật, không phải kết luận tuyệt đối.</p><p class="origin-story">${escapeHtml(item.origin_story_vi || item.etymology_note_vi || "Chưa có thuyết minh chi tiết.")}</p><div class="evolution-row">${evolutionCard("Giáp Cốt", item.oracle_image_url, "Chưa có hình")}${evolutionCard("Kim Văn", item.bronze_image_url, "Chưa có hình")}${evolutionCard("Tiểu Triện", item.seal_image_url, "Chưa có hình")}<figure><div class="evolution-image modern">${escapeHtml(item.simplified)}</div><figcaption>Khải thư</figcaption></figure></div><p class="origin-reference">Ghi chú tư liệu gốc: ${escapeHtml(item.etymology_note_vi || "Đang bổ sung")}</p><a href="${escapeHtml(item.evolution_source_url)}" target="_blank" rel="noopener">Mở hồ sơ chữ cổ và đối chiếu nguồn</a></details>
...
document.querySelector("#monoCore")?.addEventListener("change", renderMonolithicList);
